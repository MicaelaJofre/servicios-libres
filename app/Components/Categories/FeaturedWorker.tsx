  ...
  return (
    <div className="w-56 h-44 relative">
        <div className="rounded-2xl flex items-center justify-center p-1 w-fit bg-white z-10 absolute">
          <Image className="rounded-2xl relative h-[70px] w-[70px] object-cover" src={image} alt={name + 'image-profile'} width={256} height={256}></Image>
        </div>
        <div className="w-[185px] bg-[#FDFCFD] rounded-xl absolute right-3 bottom-0 p-4 pr-8 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-end gap-1">
              <div className="flex gap-0.5">{Array(5).fill(0).map((_, index) => <StarIcon key={name + '-star-' + index} width={14} height={14} color={setStarColor(index + 1, rate.average)} />)}</div>
